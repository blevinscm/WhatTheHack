param(
    ...
    [securestring]$covid19BackupStorageContainerSAS
)
...
for ($i = 1; $i -le $teamCount; $i++)
{
    ...
    New-AzResourceGroupDeployment -Name $deploymentName -ResourceGroupName $resourceGroupName -Location $region -TemplateFile $deploymentTemplateFile -TemplateParameterFile $deploymentParameterFile -sqlAdminLoginPassword $sqlAdminLoginPassword -vmAdminPassword $vmAdminPassword -covid19BackupStorageContainerSAS $covid19BackupStorageContainerSAS -AsJob    
}